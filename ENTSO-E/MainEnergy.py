...
#The timetamp syntax is YYYYMMDD, it will get the data from the current_timestamp_start date on the website.
startdk = pd.Timestamp(current_date_timestamp_start, tz='Europe/Copenhagen')
enddk = pd.Timestamp(current_date_timestamp_end, tz='Europe/Copenhagen')

startfi= pd.Timestamp(current_date_timestamp_start, tz='Europe/Helsinki')
endfi= pd.Timestamp(current_date_timestamp_end, tz='Europe/Helsinki')
...
# Extract x, y, and hour values for Finland data
finland_x = [data[0] for data in Total_Finland]
finland_y = [data[2] for data in Total_Finland]
finland_hour = [data[1] for data in Total_Finland]

# Extract x, y, and hour values for Denmark data
denmark_x = [data[0] for data in Total_Denmark]
denmark_y = [data[2] for data in Total_Denmark]
denmark_hour = [data[1] for data in Total_Denmark]

# Plot Finland data
plt.scatter(finland_x, finland_y, c=finland_hour, cmap='viridis', label='Finland')
plt.colorbar(label='Hour of the day')
plt.xlabel('Day of the year')
plt.ylabel('Energy generated')
plt.title('Energy Generation in Finland')
plt.legend()
plt.show()

# Plot Denmark data
plt.scatter(denmark_x, denmark_y, c=denmark_hour, cmap='viridis', label='Denmark')
plt.colorbar(label='Hour of the day')
plt.xlabel('Day of the year')
plt.ylabel('Energy generated')
plt.title('Energy Generation in Denmark')
plt.legend()
plt.show()